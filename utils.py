...
def create_data_on_disk(graph_size, num_samples, is_save=True, filename=None, is_return=False, seed=1234):
    ...
    graphs = tf.random.uniform(minval=0, maxval=1, shape=(num_samples, graph_size + 2, 2))
    costs = tf.convert_to_tensor([scipy.spatial.distance.cdist(g, g, "euclidean") for g in graphs])

    if is_save:
        save_to_pickle('Validation_dataset_{}.pkl'.format(filename), (graphs, costs))

    if is_return:
        return tf.data.Dataset.from_tensor_slices((list(graphs), list(costs)))
...
def read_from_pickle(path, return_tf_data_set=True, num_samples=None):
    ...
    if return_tf_data_set:
        start, end, graphs, costs = objects
        if num_samples is not None:
            return tf.data.Dataset.from_tensor_slices((list(start), list(end), list(graphs), list(costs))).take(num_samples)
        else:
            return tf.data.Dataset.from_tensor_slices((list(start), list(end), list(graphs), list(costs)))
    else:
        return objects
